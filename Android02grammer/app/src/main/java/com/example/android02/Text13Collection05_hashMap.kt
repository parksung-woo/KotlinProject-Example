...
fun main() {
    ...
    //1. 변경가능한 map : hashMapOf, mutableMap >> HashMap, MutableMap
    // key중복허용안함(value는 중복허용), 순서가 없다.
    val map:MutableMap<Any,Any?> = //<key값(보통 유니크한 값) , vlaue값 >
        mutableMapOf("key1" to "kim","key2" to 100,"key3" to 100, 1 to 111) //동일 key일 경우 마지막 key의 value로 덮어씌워진다.
